                ...
                <div class="card shadow">
                    ...
                    <div class="card-body">
                        <form id="form" method="post" action="/admin/e-course/update-chapter" enctype="multipart/form-data">
                            {{ csrf_field() }}
                            <input type="hidden" name="id" value="{{ $chapter->id_cc }}">
                            <div class="row">
                                <div class="form-group col-md-12">
                                    ...
                                    <select name="chapter_nomor" class="form-control {{ $errors->has('chapter_nomor') ? 'is-invalid' : '' }}">
                                        ...
                                        @for($i=0; $i<=100; $i++)
										<option value="{{ $i }}" {{ $chapter->chapter_nomor == $i ? 'selected' : '' }} {{ in_array($i, $array) ? 'disabled' : '' }}>Chapter #{{ $i }}</option>
                                        @endfor
									</select>
                                ...
                                <div class="form-group col-md-12">
                                    ...
                                    <input type="text" name="judul" class="form-control {{ $errors->has('judul') ? 'is-invalid' : '' }}" value="{{ $chapter->chapter_judul }}" placeholder="Masukkan Judul">
                                    @if($errors->has('judul'))
                                ...
                                <div class="form-group col-md-12">
									...
									<br>
									<img id="img-file" src="{{ $chapter->chapter_icon != '' ? asset('assets/images/chapter/'.$chapter->chapter_icon) : '' }}" class="mt-2 img-thumbnail {{ $chapter->chapter_icon != '' ? '' : 'd-none' }}" style="max-height: 150px">
									<input type="hidden" name="gambar" id="src-img">
                                </div>
                                <div class="form-group col-md-12">
                                    <label>Kode Voucher</label>
                                    <input type="text" name="voucher" class="form-control {{ $errors->has('voucher') ? 'is-invalid' : '' }}" value="{{ $chapter->voucher }}" placeholder="Masukkan Kode Voucher">
                                    <small class="text-muted">Kosongi saja jika tidak akan menggunakan voucher.</small>
                                </div>
                            </div>
                        </form>
                    </div>
