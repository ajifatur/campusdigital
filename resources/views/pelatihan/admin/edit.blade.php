										...
										<table class="table table-bordered" id="table-materi">
											<tbody>
												...
												<tr data-id="{{ ($key+1) }}">
													...
													<td width="50"><button class="btn btn-danger btn-block btn-delete-materi {{ count($pelatihan->materi_pelatihan) <= 1 ? 'd-none' : '' }}" data-id="{{ ($key+1) }}" title="Hapus"><i class="fa fa-trash"></i></button></td>
												</tr>
												@endforeach
											</tbody>
										</table>
...
<script type="text/javascript">
	$(document).ready(function(){
		...
		$("input[name=tanggal_sertifikat]").daterangepicker({
			...
			timePicker24Hour: true,
    		showDropdowns: true,
			startDate: "{{ generate_date_range('join', $pelatihan->tanggal_sertifikat_from.' - '.$pelatihan->tanggal_sertifikat_to)['from'] }}",
			endDate: "{{ generate_date_range('join', $pelatihan->tanggal_sertifikat_from.' - '.$pelatihan->tanggal_sertifikat_to)['to'] }}",
			locale: {
			  format: 'DD/MM/YYYY HH:mm'
			}
		});
